...
G_DEFINE_TYPE (NldSearchContextPicker, nld_search_context_picker,
	       GTK_TYPE_BUTTON)

     static void
      
	     nld_search_context_picker_class_init (NldSearchContextPickerClass
						   *
						   nld_search_context_picker_class)
{
	GtkButtonClass *button_class =
		GTK_BUTTON_CLASS (nld_search_context_picker_class);
	...
	signals [CONTEXT_CHANGED] =
		g_signal_new ("context-changed",
			      G_TYPE_FROM_CLASS
			      (nld_search_context_picker_class),
			      G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (NldSearchContextPickerClass,
...
{
	NldSearchContextPickerPrivate *priv =
		NLD_SEARCH_CONTEXT_PICKER_GET_PRIVATE (picker);
	GtkBox *hbox;
	...
	gtk_box_pack_start (hbox, GTK_WIDGET (priv->cur_icon), FALSE, FALSE,
			    0);
...
static void
menu_position_func (GtkMenu * menu, int *x, int *y,
		    gboolean * push_in, gpointer picker)
{
...
{
	NldSearchContextPickerPrivate *priv =
		NLD_SEARCH_CONTEXT_PICKER_GET_PRIVATE (button);

	gtk_menu_popup (GTK_MENU (priv->menu), NULL, NULL,
			menu_position_func, button, 1,
...
item_activated (GtkMenuItem * item, gpointer picker)
{
	...
	image = GTK_IMAGE (gtk_image_menu_item_get_image
...
void
nld_search_context_picker_add_context (NldSearchContextPicker * picker,
				       const char *label,
...
void
nld_search_context_picker_set_context (NldSearchContextPicker * picker,
				       int context_id)
{
	NldSearchContextPickerPrivate *priv =
		NLD_SEARCH_CONTEXT_PICKER_GET_PRIVATE (picker);
