...
static gboolean app_resizer_paint_window (GtkWidget * widget,
...
app_resizer_get_type (void)
{
	...
	if (!object_type) {
		...
		object_type = g_type_register_static (GTK_TYPE_LAYOUT,
						      "AppResizer",
						      &object_info, 0);
	}
...
void
app_resizer_layout_table_default (AppResizer * widget, GtkTable * table,
				  GList * element_list)
{
...
relayout_tables (AppResizer * widget, gint num_cols)
{
	...
	for (table_list = widget->cached_tables_list; table_list != NULL;
		...
		launcher_list = g_list_reverse (launcher_list);	/* FIXME -
								   ugly hack
								   because
								   table
								   stores
...
calculate_num_cols (AppResizer * resizer, gint avail_width)
{
	if (resizer->table_elements_homogeneous) {
		if (resizer->cached_element_width == -1) {
			GtkTable *table =
				GTK_TABLE (resizer->cached_tables_list->data);
			GtkWidget *table_element =
				GTK_WIDGET (gtk_container_get_children
					    (GTK_CONTAINER (table))->data);
			resizer->cached_element_width =
				table_element->allocation.width;
...
static gint
relayout_tables_if_needed (AppResizer * widget, gint avail_width,
			   gint current_num_cols)
{
	gint num_cols = calculate_num_cols (widget, avail_width);

	if (num_cols < 1) {
		num_cols = 1;	/* just horiz scroll if avail_width is less
				   than one column */
	}
...
app_resizer_size_allocate (GtkWidget * widget, GtkAllocation * allocation)
{
	...
	if (!resizer->cached_tables_list) {	/* if everthing is currently
						   filtered out - just return 
						 */
		if (GTK_WIDGET_CLASS (parent_class)->size_allocate)
			(*GTK_WIDGET_CLASS (parent_class)->
			 size_allocate) (widget, allocation);

		/* We want the message to center itself and only scroll if
		   it's bigger than the available real size. */
		GtkAllocation child_allocation;

		child_allocation.x = 0;
		child_allocation.y = 0;
		child_allocation.width =
			MAX (allocation->width, child->requisition.width);
		child_allocation.height =
			MAX (allocation->height, child->requisition.height);
	...
	gint useable_area = allocation->width -
		(child->requisition.width -
		 GTK_WIDGET (resizer->cached_tables_list->data)->requisition.
		 width);
	new_num_cols =
		relayout_tables_if_needed (APP_RESIZER (resizer),
					   useable_area,
					   resizer->cur_num_cols);
	if (resizer->cur_num_cols != new_num_cols) {
		GtkRequisition req;

		gtk_widget_size_request (child, &req);	/* Have to do this so 
							   that it requests,
							   and thus gets
							   allocated, new
							   amount */
...
GtkWidget *
app_resizer_new (GtkVBox * child, gint initial_num_columns,
		 gboolean homogeneous, AppShellData * app_data)
{
	...
	g_signal_connect (G_OBJECT (widget), "expose-event",
			  G_CALLBACK (app_resizer_paint_window), app_data);
...
gboolean
app_resizer_paint_window (GtkWidget * widget, GdkEventExpose * event,
			  AppShellData * app_data)
{
	...
	if (app_data->selected_group) {
		GtkWidget *selected_widget =
			GTK_WIDGET (app_data->selected_group);

		gdk_draw_rectangle (
			/* drawing on child window and child coordinates */
			selected_widget->window,
			selected_widget->style->light_gc [GTK_STATE_SELECTED],
			TRUE,
			selected_widget->allocation.x, selected_widget->allocation.y,
			/* drawing with our coordinates here to draw all the
			 * way to the edge.  */
