...
static GObject *
nameplate_tile_constructor (GType type, guint n_param,
			    GObjectConstructParam * param)
{
	GObject *g_obj =
		(*G_OBJECT_CLASS (nameplate_tile_parent_class)->
		 constructor) (type, n_param, param);
...
static void
nameplate_tile_get_property (GObject * g_object, guint prop_id,
...
static void
nameplate_tile_set_property (GObject * g_object, guint prop_id,
			     const GValue * value, GParamSpec * param_spec)
{
	...
	case PROP_NAMEPLATE_SUBHEADER:
		if (GTK_IS_WIDGET (widget_obj)) {
			...
			gtk_container_add (priv->subheader_ctnr,
					   this->subheader);
...
nameplate_tile_setup (NameplateTile * this)
{
	...
	priv->image_ctnr =
		GTK_CONTAINER (gtk_alignment_new (0.5, 0.5, 1.0, 1.0));
	priv->header_ctnr =
		GTK_CONTAINER (gtk_alignment_new (0.0, 0.5, 1.0, 1.0));
	priv->subheader_ctnr =
		GTK_CONTAINER (gtk_alignment_new (0.0, 0.5, 1.0, 1.0));
	...
	gtk_box_pack_start (GTK_BOX (hbox), GTK_WIDGET (priv->image_ctnr),
			    FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (hbox), alignment, TRUE, TRUE, 0);

	gtk_container_add (GTK_CONTAINER (alignment), vbox);
	gtk_box_pack_start (GTK_BOX (vbox), GTK_WIDGET (priv->header_ctnr),
			    FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (vbox), GTK_WIDGET (priv->subheader_ctnr),
			    FALSE, FALSE, 0);
...
nameplate_tile_drag_begin (GtkWidget * widget, GdkDragContext * context)
{
	...
	(*GTK_WIDGET_CLASS (nameplate_tile_parent_class)->drag_begin) (widget,
								       context);
