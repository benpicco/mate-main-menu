...
#define SYSTEM_TILE_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYSTEM_TILE_TYPE, SystemTilePrivate))
     ...
     static void
       system_tile_class_init (SystemTileClass * sys_tile_class)
{
...
system_tile_new_with_type (SystemTileType type, MainMenuConf * conf)
{
	...
	case SYSTEM_TILE_TYPE_HELP:
		...
		uri = g_strdup (gnome_desktop_item_get_location
				(desktop_item));
...
load_image (SystemTile * tile)
{
	...
	priv->image_is_broken =
		load_image_by_id (GTK_IMAGE (NAMEPLATE_TILE (tile)->image),
				  GTK_ICON_SIZE_MENU, priv->image_id);
}
...
system_tile_open (Tile * tile, TileEvent * event, TileAction * action)
{
	...
	case SYSTEM_TILE_TYPE_CONTROL_CENTER:
	case SYSTEM_TILE_TYPE_PACKAGE_MANAGER:
		open_desktop_item_exec (SYSTEM_TILE_GET_PRIVATE (tile)->
					desktop_item);
		break;
...
system_tile_lock_screen (Tile * tile, TileEvent * event, TileAction * action)
{
	...
	command_priority =
		get_slab_gconf_slist (SLAB_LOCK_SCREEN_PRIORITY_KEY);
	...
	for (node = command_priority; node; node = node->next) {
		...
		if (cmd_string) {
			exec_string =
				g_strdup_printf ("%s %s", cmd_string,
						 arg_string);

			g_spawn_async (NULL,
				       g_strsplit (exec_string, " ", 0),
