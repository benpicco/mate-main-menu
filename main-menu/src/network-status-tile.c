...
#define NETWORK_STATUS_TILE_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), NETWORK_STATUS_TILE_TYPE, NetworkStatusTilePrivate))
     ...
     static void
	     ...
	     network_status_tile_class_init (NetworkStatusTileClass *
					     net_tile_class)
{
	...
	g_type_class_add_private (net_tile_class,
				  sizeof (NetworkStatusTilePrivate));
}
...
network_status_tile_new ()
{
	...
	tile = g_object_new (NETWORK_STATUS_TILE_TYPE,
			     "tile-uri", "tile://network-status",
...
{
	NetworkStatusTilePrivate *priv =
		NETWORK_STATUS_TILE_GET_PRIVATE (tile);
...
{
	NetworkStatusTilePrivate *priv =
		NETWORK_STATUS_TILE_GET_PRIVATE (g_object);
	if (priv->status_info)
	...
	(*G_OBJECT_CLASS (network_status_tile_parent_class)->
	 finalize) (g_object);
}
...
network_status_tile_open (Tile * tile, TileEvent * event, TileAction * action)
{
	...
	gtk_window_present_with_time (GTK_WINDOW (priv->info_dialog),
				      event->time);
}
...
update_tile (NetworkStatusTile * tile)
{
	...
	if (!priv->status_info) {
		...
		case DEVICE_TYPE_802_11_WIRELESS:
			markup = g_strdup_printf (_("Connected to: %s"),
						  priv->status_info->essid);
	...
	slab_load_image (GTK_IMAGE (NAMEPLATE_TILE (tile)->image),
			 GTK_ICON_SIZE_BUTTON, icon_name);
	gtk_label_set_text (GTK_LABEL (NAMEPLATE_TILE (tile)->header),
			    header_text);
...
refresh_status (NetworkStatusTile * tile)
{
	...
	if (!priv->agent) {
		...
		g_signal_connect (G_OBJECT (priv->agent),
				  "status-changed",
	...
	if (priv->agent) {
		...
		priv->status_info =
			network_status_agent_get_first_active_device_info
			(priv->agent);
	}
}
...
build_info_dialog (NetworkStatusTile * tile)
{
	...
	priv->info_dialog_xml =
		glade_xml_new (filename, "info_dialog", "NetworkManager");

	priv->info_dialog =
		glade_xml_get_widget (priv->info_dialog_xml, "info_dialog");

	close_button =
		glade_xml_get_widget (priv->info_dialog_xml, "closebutton1");
	net_cfg_button =
		glade_xml_get_widget (priv->info_dialog_xml,
				      "configure_button");

	g_signal_connect (priv->info_dialog,
			  "delete-event",
	...
	g_signal_connect (net_cfg_button,
			  "clicked",
			  G_CALLBACK (info_dialog_cfg_button_clicked_cb),
			  priv->info_dialog);
...
update_info_dialog (NetworkStatusTile * tile)
{
	...
	case DEVICE_TYPE_802_11_WIRELESS:
		iface_and_type =
			g_strdup_printf (_("Wireless Ethernet (%s)"),
					 priv->status_info->iface);
		break;

	case DEVICE_TYPE_802_3_ETHERNET:
		iface_and_type =
			g_strdup_printf (_("Wired Ethernet (%s)"),
					 priv->status_info->iface);
		break;
	...
	if (priv->status_info->speed_mbs)
		speed = g_strdup_printf (_("%d Mb/s"),
					 priv->status_info->speed_mbs);
	else
	...
	set_glade_label (xml, "label-ip-address",
			 priv->status_info->ip4_addr);
	set_glade_label (xml, "label-broadcast-address",
			 priv->status_info->broadcast);
	set_glade_label (xml, "label-subnet-mask",
			 priv->status_info->subnet_mask);
	set_glade_label (xml, "label-default-route",
			 priv->status_info->route);
	set_glade_label (xml, "label-primary-dns",
			 priv->status_info->primary_dns);
	set_glade_label (xml, "label-secondary-dns",
			 priv->status_info->secondary_dns);
	set_glade_label (xml, "label-hardware-address",
			 priv->status_info->hw_addr);
}
...
launch_network_config ()
{
	GnomeDesktopItem *desktop_item =
		load_desktop_item_from_gconf_key
		(SLAB_NETWORK_CONFIG_TOOL_KEY);
