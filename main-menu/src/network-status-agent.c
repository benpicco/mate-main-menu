...
static NetworkStatusInfo *nm_get_device_info (NetworkStatusAgent *,
					      DBusGProxy *);

static void nm_state_change_cb (DBusGProxy *, guint, gpointer);
static DBusHandlerResult nm_message_filter (DBusConnection *, DBusMessage *,
					    gpointer);
...
G_DEFINE_TYPE (NetworkStatusAgent, network_status_agent, G_TYPE_OBJECT)
     ...
     static void
       network_status_agent_class_init (NetworkStatusAgentClass * this_class)
{
	GObjectClass *g_obj_class = (GObjectClass *) this_class;

	g_obj_class->dispose = network_status_agent_dispose;

	network_status_agent_signals [STATUS_CHANGED] =
		g_signal_new ("status-changed",
			      G_TYPE_FROM_CLASS (this_class),
			      G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (NetworkStatusAgentClass,
					       status_changed), NULL, NULL,
			      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

	g_type_class_add_private (this_class,
				  sizeof (NetworkStatusAgentPrivate));
}

static void
network_status_agent_init (NetworkStatusAgent * agent)
{
	NetworkStatusAgentPrivate *priv =
		NETWORK_STATUS_AGENT_GET_PRIVATE (agent);
...
init_nm_connection (NetworkStatusAgent * agent)
{
	...
	if (!priv->nm_conn) {
		handle_g_error (&error, "%s: dbus_g_bus_get () failed",
				__FUNCTION__);
	...
	if (!dbus_bus_name_has_owner
	    (dbus_g_connection_get_connection (priv->nm_conn),
	...
	dbus_connection_set_exit_on_disconnect
		(dbus_g_connection_get_connection (priv->nm_conn), FALSE);

	priv->nm_proxy = dbus_g_proxy_new_for_name (priv->nm_conn,
						    NM_DBUS_SERVICE,
						    NM_DBUS_PATH,
						    NM_DBUS_INTERFACE);

	dbus_g_proxy_add_signal (priv->nm_proxy,
				 NM_DBUS_SIGNAL_STATE_CHANGE,
				 G_TYPE_UINT, G_TYPE_INVALID);

	dbus_g_proxy_connect_signal (priv->nm_proxy,
				     NM_DBUS_SIGNAL_STATE_CHANGE,
				     G_CALLBACK (nm_state_change_cb),
				     agent, NULL);

	dbus_connection_add_filter (dbus_g_connection_get_connection
				    (priv->nm_conn), nm_message_filter, agent,
				    NULL);
}
...
nm_get_devices (NetworkStatusAgent * agent)
{
	...
	dbus_g_proxy_call (priv->nm_proxy, "getDevices", &error,
			   G_TYPE_INVALID,
			   dbus_g_type_get_collection ("GPtrArray",
						       DBUS_TYPE_G_PROXY),
			   &ptr_array, G_TYPE_INVALID);

	if (error) {
		handle_g_error (&error, "%s: calling \"getDevices\" failed",
				__FUNCTION__);
...
nm_get_device_info (NetworkStatusAgent * agent, DBusGProxy * device)
{
	...
	NetworkStatusInfo *info =
		g_object_new (NETWORK_STATUS_INFO_TYPE, NULL);
	...
	proxy = dbus_g_proxy_new_for_name (priv->nm_conn,
					   NM_DBUS_SERVICE,
	...
	if (error) {
		handle_g_error (&error,
				"%s: calling \"getProperties\" (A) failed",
				__FUNCTION__);
	...
	if (info->active) {
		dbus_g_proxy_call (proxy, "getDriver", &error,
				   G_TYPE_INVALID,
				   G_TYPE_STRING, &info->driver,
				   G_TYPE_INVALID);

		if (error)
			handle_g_error (&error,
					"%s: calling \"getDriver\" failed",
					__FUNCTION__);
		...
		if (error)
			handle_g_error (&error,
					"%s: calling \"getProperties\" (B) failed",
					__FUNCTION__);
	}
...
nm_message_filter (DBusConnection * nm_conn, DBusMessage * msg,
		   gpointer user_data)
{
...
string_is_valid_dbus_path (const gchar * str)
{
	...
	while (s != end) {
		if (*s == '/') {
			if ((s - last_slash) < 2)
				return FALSE;	/* no empty path components
						   allowed */
	...
	if ((end - last_slash) < 2 && len > 1)
		return FALSE;	/* trailing slash not allowed unless the
				   string is "/" */
...
gtop_get_first_active_device_info ()
{
	...
	for (i = 0; i < net_list.number; ++i) {
		...
		if (CHECK_FLAG (net_load.if_flags, GLIBTOP_IF_FLAGS_RUNNING)
			...
			ret = iw_get_basic_config (sock_fd, networks [i],
						   &wl_cfg);
