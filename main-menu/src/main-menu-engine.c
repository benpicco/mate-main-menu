...
main_menu_engine_class_init (MainMenuEngineClass * main_menu_engine_class)
{
	...
	g_type_class_add_private (main_menu_engine_class,
				  sizeof (MainMenuEnginePrivate));
}
...
main_menu_engine_search_available (MainMenuEngine * engine)
{
	...
	if (!path_is_on_block_device
	    || g_getenv ("BEAGLE_SYNCHRONIZE_LOCALLY") != NULL) {
		remote_storage_dir =
			g_build_filename (beagle_home, ".beagle",
		...
		if (!g_file_get_contents
		    (remote_storage_dir, &socket_dir, NULL, NULL)) {
			g_free (remote_storage_dir);
		...
		if (!g_file_test
		    (socket_dir, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)) {
			g_free (socket_dir);
...
void
main_menu_engine_execute_search (MainMenuEngine * engine,
				 const gchar * search_string)
{
	...
	g_string_append (cmd,
			 &cmd_template [pivot + strlen ("SEARCH_STRING")]);
...
void
main_menu_engine_add_user_app (MainMenuEngine * engine,
			       const gchar * desktop_uri)
{
	...
	if (!main_menu_conf_find_user_app_by_uri (priv->conf, desktop_uri)) {
		priv->conf->file_area_conf->user_specified_apps =
			g_list_append (priv->conf->file_area_conf->
				       user_specified_apps,
				       g_strdup (desktop_uri)
			);

		main_menu_conf_sync (priv->conf,
				     priv->conf->file_area_conf->
				     user_specified_apps);
	}
}
