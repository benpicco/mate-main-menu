...
static void bonobo_menu_menu_open_cb (BonoboUIComponent *, gpointer,
				      const gchar *);
static void bonobo_menu_menu_about_cb (BonoboUIComponent *, gpointer,
				       const gchar *);
static void applet_change_orient_cb (PanelApplet *, PanelAppletOrient,
				     gpointer);
static void applet_change_background_cb (PanelApplet *,
...
static gboolean main_window_key_press_cb (GtkWidget *, GdkEventKey *,
					  gpointer);
static gboolean main_window_button_press_cb (GtkWidget *, GdkEventButton *,
					     gpointer);
...
static gboolean main_window_grab_broken_cb (GtkWidget *, GdkEvent *,
					    gpointer);
static gboolean grabbing_window_event_cb (GtkWidget *, GdkEvent *, gpointer);

static void tile_drag_data_rcv_cb (GtkWidget *,
				   GdkDragContext *,
				   gint, gint,
				   GtkSelectionData *,
				   guint, guint, gpointer);

static gboolean panel_button_button_press_cb (GtkWidget *, GdkEventButton *,
					      gpointer);
static gboolean panel_button_enter_notify_cb (GtkWidget *, GdkEventButton *,
					      gpointer);

static GtkWidget *create_search_widget (MainMenuUI *, GtkSizeGroup *);

static GtkWidget *create_system_table_widget (MainMenuUI *, GtkSizeGroup *,
					      GtkSizeGroup *);
...
static void section_header_label_style_set (GtkWidget *, GtkStyle *,
					    gpointer);
...
main_menu_ui_class_init (MainMenuUIClass * main_menu_ui_class)
{
	...
	g_type_class_add_private (main_menu_ui_class,
				  sizeof (MainMenuUIPrivate));
}
...
slab_action_filter (GdkXEvent * gdk_xevent,
...
MainMenuUI *
main_menu_ui_new (PanelApplet * applet, MainMenuConf * conf,
		  MainMenuEngine * engine)
{
	MainMenuUI *ui;
	...
	g_signal_connect (G_OBJECT (priv->applet),
			  "change_orient",
	...
	/* install message client handler to get messages through the X root
	   window */
	gdk_atom_slab_action = gdk_atom_intern ("_SLAB_ACTION", FALSE);
	atom_slab_main_menu_action =
		XInternAtom (GDK_DISPLAY_XDISPLAY
			     (gdk_display_get_default ()),
			     "_SLAB_ACTION_MAIN_MENU", FALSE);
	gdk_display_add_client_message_filter (gdk_display_get_default (),
					       gdk_atom_slab_action,
					       slab_action_filter, ui);

	panel_applet_setup_menu_from_file (priv->applet,
					   NULL,
					   "GNOME_MainMenu_ContextMenu.xml",
...
static void
bonobo_menu_menu_open_cb (BonoboUIComponent * component, gpointer user_data,
			  const gchar * verb)
{
...
static void
bonobo_menu_menu_about_cb (BonoboUIComponent * component, gpointer user_data,
			   const gchar * verb)
{
	...
	if (!priv->about_dialog) {
		...
		g_signal_connect (G_OBJECT (priv->about_dialog),
				  "delete-event",
...
static void
applet_change_orient_cb (PanelApplet * applet, PanelAppletOrient orient,
			 gpointer user_data)
{
			     ...
			     GdkPixmap * pixmap, gpointer user_data)
{
	...
	case PANEL_COLOR_BACKGROUND:
		gtk_widget_modify_bg (GTK_WIDGET (priv->applet),
	...
	/* Alas, I don't know why this is necessary, but otherwise we get
	   garbage the first time it draws. */
...
build_main_menu_window (MainMenuUI * ui)
{
	...
	priv->file_area =
		FILE_AREA_WIDGET (file_area_widget_new (ui, priv->engine));

	search_show_label_group =
		gtk_size_group_new (GTK_SIZE_GROUP_HORIZONTAL);

	search_widget = create_search_widget (ui, search_show_label_group);

	gtk_size_group_add_widget (search_show_label_group,
				   priv->file_area->selector_label);
	...
	if (priv->conf->lock_down_conf->search_area_visible)
		gtk_box_pack_start (GTK_BOX (left_pane), search_widget, FALSE,
				    FALSE, 0);

	gtk_box_pack_start (GTK_BOX (left_pane), GTK_WIDGET (priv->file_area),
			    FALSE, FALSE, 0);
	...
	system_table_widget =
		create_system_table_widget (ui, icon_group, label_group);
	...
	if (hd_tile) {
		g_signal_connect (G_OBJECT (hd_tile),
				  "tile-action-triggered",
				  G_CALLBACK (tile_action_triggered_cb), ui);

		gtk_box_pack_start (GTK_BOX (status_section), hd_tile, FALSE,
				    FALSE, 0);
	}
	...
	if (net_tile) {
		g_signal_connect (G_OBJECT (net_tile),
				  "tile-action-triggered",
				  G_CALLBACK (tile_action_triggered_cb), ui);

		gtk_box_pack_start (GTK_BOX (status_section), net_tile, FALSE,
				    FALSE, 0);
	}
	...
	if (priv->conf->lock_down_conf->system_area_visible) {
		gtk_box_pack_start (GTK_BOX (right_pane), system_header,
				    FALSE, FALSE, 0);
		gtk_box_pack_start (GTK_BOX (right_pane), system_table_widget,
				    FALSE, FALSE, 0);
	}

	if (priv->conf->lock_down_conf->status_area_visible) {
		gtk_box_pack_end (GTK_BOX (right_pane), status_section, FALSE,
				  FALSE, 0);
		gtk_box_pack_end (GTK_BOX (right_pane), status_header, FALSE,
				  FALSE, 0);
	}

	slab_window_set_contents (SLAB_WINDOW (window), left_pane,
				  right_pane);
	...
	g_signal_connect (G_OBJECT (window),
			  "size_allocate",
			  G_CALLBACK (main_window_resize_cb), ui);
	...
	g_signal_connect (G_OBJECT (window),
			  "map_event", G_CALLBACK (main_window_map_cb), ui);

	g_signal_connect (G_OBJECT (window),
			  "unmap_event",
			  G_CALLBACK (main_window_unmap_cb), ui);
...
build_panel_button (MainMenuUI * ui)
{
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->panel_button),
				      FALSE);
	...
	g_object_set_data (G_OBJECT (priv->panel_button),
			   "double-click-detector",
			   double_click_detector_new ()
		);

	g_signal_connect (G_OBJECT (priv->panel_button),
			  "button_press_event",
	...
	gtk_drag_dest_set (GTK_WIDGET (priv->panel_button),
			   GTK_DEST_DEFAULT_ALL,
			   NULL, 0, GDK_ACTION_COPY | GDK_ACTION_MOVE);
...
reorient_panel_button (MainMenuUI * ui)
{
	...
	icon = gtk_image_new_from_icon_name ("gnome-fs-client",
					     GTK_ICON_SIZE_MENU);
...
static void
main_window_unmap_cb (GtkWidget * widget, GdkEvent * event,
		      gpointer user_data)
{
...
static void
main_window_resize_cb (GtkWidget * widget, GtkAllocation * alloc,
		       gpointer user_data)
{
...
static gboolean
main_window_grab_broken_cb (GtkWidget * widget, GdkEvent * event,
			    gpointer user_data)
{
	...
	if (grab_event->grab_window) {
		gdk_window_get_user_data (grab_event->grab_window,
					  &window_data);

		if (GTK_IS_WIDGET (window_data))
			g_signal_connect (G_OBJECT (window_data),
					  "event",
					  G_CALLBACK
					  (grabbing_window_event_cb),
					  user_data);
	}
...
static gboolean
grabbing_window_event_cb (GtkWidget * widget, GdkEvent * event,
			  gpointer user_data)
{
...
static void
tile_drag_data_rcv_cb (GtkWidget * widget,
...
static gboolean
main_window_button_press_cb (GtkWidget * widget, GdkEventButton * event,
			     gpointer user_data)
{
	MainMenuUI *ui = MAIN_MENU_UI (user_data);
	MainMenuUIPrivate *priv = MAIN_MENU_UI_GET_PRIVATE (ui);
...
static gboolean
panel_button_button_press_cb (GtkWidget * button, GdkEventButton * event,
			      gpointer user_data)
{
	...
	detector =
		DOUBLE_CLICK_DETECTOR (g_object_get_data
				       (G_OBJECT (button),
					"double-click-detector"));

	if (double_click_detector_is_double_click
	    (detector, event->time, TRUE))
		return TRUE;
...
static gboolean
panel_button_enter_notify_cb (GtkWidget * button, GdkEventButton * event,
			      gpointer user_data)
{
	return TRUE;
}
...
show_main_window (MainMenuUI * ui, guint32 timestamp)
{
	...
	gtk_window_present_with_time (GTK_WINDOW (priv->main_window),
				      timestamp);
...
position_main_window (MainMenuUI * ui)
{
	...
	gdk_window_get_origin (panel_widget->window, &panel_widget_alloc.x,
			       &panel_widget_alloc.y);
...
static GtkWidget *
create_system_table_widget (MainMenuUI * ui, GtkSizeGroup * icon_group,
			    GtkSizeGroup * label_group)
{
	MainMenuUIPrivate *priv = MAIN_MENU_UI_GET_PRIVATE (ui);
	...
	disable_lock_screen =
		GPOINTER_TO_INT (get_gconf_value
				 (DISABLE_LOCK_SCREEN_GCONF_KEY));
	disable_log_out =
		GPOINTER_TO_INT (get_gconf_value (DISABLE_LOG_OUT_GCONF_KEY));

	for (node = type_list; node; node = node->next) {
		type = GPOINTER_TO_INT (node->data);

		if (disable_lock_screen
		    && type == SYSTEM_TILE_TYPE_LOCK_SCREEN)
			continue;
		...
		if (tile) {
			...
			gtk_box_pack_start (GTK_BOX (table), alignment, TRUE,
					    TRUE, 0);

			g_signal_connect (G_OBJECT (tile),
					  "tile-action-triggered",
					  G_CALLBACK
					  (tile_action_triggered_cb), ui);
...
static void
section_header_label_style_set (GtkWidget * widget, GtkStyle * prev_style,
				gpointer user_data)
{
	if (prev_style
...
grab_focus (MainMenuUI * ui, GdkEvent * event)
{
	...
	if (priv->conf->urgent_close) {
		...
		status = gdk_pointer_grab (priv->main_window->window,
					   TRUE, GDK_BUTTON_PRESS_MASK,
					   NULL, NULL, time);

		priv->ptr_is_grabbed = (status == GDK_GRAB_SUCCESS);

		status = gdk_keyboard_grab (priv->main_window->window, TRUE,
					    time);
...
search_entry_activate_cb (GtkEntry * entry_widget, gpointer user_data)
{
	...
	main_menu_engine_execute_search (priv->engine,
					 gtk_entry_get_text (entry_widget));
}

static void
tile_action_triggered_cb (Tile * tile, TileEvent * event, TileAction * action,
			  gpointer user_data)
{
	MainMenuUI *ui = MAIN_MENU_UI (user_data);
	MainMenuUIPrivate *priv = MAIN_MENU_UI_GET_PRIVATE (ui);
...
bind_search_key (void)
{
	...
	path = g_build_filename (g_get_home_dir (),
				 ".beagle/config/searching.xml", NULL);
	...
	for (node = doc->children->children; node; node = node->next) {
		if (!node->name
		    || strcmp ((char *) node->name,
			       "ShowSearchWindowBinding") != 0)
			continue;
		if (!node->children)
			break;
		...
		for (node = node->children; node; node = node->next) {
			if (!node->name
			    || strcmp ((char *) node->name, "Key") != 0)
				continue;
			val = xmlNodeGetContent (node);

			key = g_strdup_printf ("%s%s%s", ctrl ? "<Ctrl>" : "",
					       alt ? "<Alt>" : "", val);
			xmlFree (val);
