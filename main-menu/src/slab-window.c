...
slab_window_new ()
{
	...
	gtk_container_set_border_width (GTK_CONTAINER (this->_hbox),
					SLAB_WINDOW_BORDER_WIDTH);
	gtk_container_add (GTK_CONTAINER (this), GTK_WIDGET (this->_hbox));

	g_signal_connect (G_OBJECT (this),
			  ...
			  GtkWidget * right_pane)
{
	...
	gtk_alignment_set_padding (GTK_ALIGNMENT (slab->_left_pane), 15, 15,
				   15, 15);
	gtk_alignment_set_padding (GTK_ALIGNMENT (slab->_right_pane), 15, 15,
				   15, 15);
...
slab_window_size_allocate (GtkWidget * widget, GtkAllocation * alloc)
{
	...
	if (GDK_IS_WINDOW (widget->window))
		gdk_window_resize (widget->window, alloc->width,
				   alloc->height);
...
paint_window (GtkWidget * widget, GdkEventExpose * event, gpointer data)
{
	...
	gdk_draw_rectangle (widget->window,
			    ...
			    FALSE,
			    widget->allocation.x + SLAB_WINDOW_BORDER_WIDTH -
			    1,
			    widget->allocation.y + SLAB_WINDOW_BORDER_WIDTH -
			    1,
			    widget->allocation.width -
			    2 * SLAB_WINDOW_BORDER_WIDTH + 1,
