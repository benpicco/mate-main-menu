...
static void themed_icon_get_property (GObject *, guint, GValue *,
				      GParamSpec *);
...
G_DEFINE_TYPE (ThemedIcon, themed_icon, GTK_TYPE_IMAGE)
     ...
     static void
       themed_icon_class_init (ThemedIconClass * themed_icon_class)
{
	...
	g_type_class_add_private (themed_icon_class,
				  sizeof (ThemedIconPrivate));
...
{
	GtkWidget *icon = GTK_WIDGET (g_object_new (THEMED_ICON_TYPE,
						    "icon-id", id,
						    "icon-size", size,
						    NULL));
...
static void
themed_icon_get_property (GObject * g_obj, guint prop_id, GValue * value,
...
static void
themed_icon_set_property (GObject * g_obj, guint prop_id,
...
themed_icon_show (GtkWidget * widget)
{
	...
	if (!priv->icon_loaded)
		priv->icon_loaded =
			load_image_by_id (GTK_IMAGE (icon), icon->size,
					  icon->id);
