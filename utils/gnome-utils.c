...
load_image_by_id (GtkImage * image, GtkIconSize size, const gchar * image_id)
{
	...
	if (g_path_is_absolute (id)) {
		...
		if (icon_exists) {
			...
			gtk_image_set_from_stock (image, "gtk-missing-image",
						  size);
	}
	else {
...
get_gconf_value (const gchar * key)
{
	...
	if (error) {
		handle_g_error (&error, "%s: error getting %s", __FUNCTION__,
				key);
...
set_gconf_value (const gchar * key, gconstpointer data)
{
	...
	list_type =
		(type ==
		 GCONF_VALUE_LIST ? gconf_value_get_list_type (value) :
		 GCONF_VALUE_INVALID);
	...
	case GCONF_VALUE_LIST:
		for (node = (GList *) data; node; node = node->next) {
			...
			if (list_type == GCONF_VALUE_STRING)
				gconf_value_set_string (value_i,
							(const gchar *) node->
							data);
			else if (list_type == GCONF_VALUE_INT)
				gconf_value_set_int (value_i,
	...
	if (error)
		handle_g_error (&error, "%s: error setting %s", __FUNCTION__,
				key);
...
guint
connect_gconf_notify (const gchar * key, GConfClientNotifyFunc cb,
		      gpointer user_data)
{
	GConfClient *client;
	guint conn_id;
	...
	client = gconf_client_get_default ();
	conn_id =
		gconf_client_notify_add (client, key, cb, user_data, NULL,
					 &error);

	if (error)
		handle_g_error (&error, "%s: error adding notify for (%s)",
				__FUNCTION__, key);
...
get_main_menu_section_header (const gchar * markup)
{
	...
	g_signal_connect (G_OBJECT (label),
			  "style-set",
			  G_CALLBACK (section_header_style_set), NULL);
...
static void
section_header_style_set (GtkWidget * widget, GtkStyle * prev_style,
			  gpointer user_data)
{
	if (prev_style
